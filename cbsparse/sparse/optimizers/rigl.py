...
class RigLMaskOptimizer(DynamicMaskOptimizer):
    r"""Implements RigL.

    `Rigging the Lottery: Making All Tickets Winners`__.
    ...
    __ https://arxiv.org/abs/1911.11134
    ...
    def update_mask(self, p, fused_p, drop_fraction, layer_idx=None):
        if self.backward_hook_objects[layer_idx].dense_grad is None:
            raise RuntimeError(
                'parameter mask cannot be updated without parameter grad'
            )
        ...
        if self.mask_init_method in ['random_balanced', 'topk_balanced']:
            ...
            if layer_type in ['query', 'key', 'value']:
                ...
                for num_head in range(self.config.num_attention_heads):
                    ...
                    # connections.
                    head_score_grow = torch.abs(
                        self.backward_hook_objects[layer_idx].dense_grad[
                            _start:_end, :
                        ]
                    )
